...
public class MedicinePgSqlControllerTest extends AbstractJwtTest<MedicineDto>{
    ...
    void add() throws Exception {
        actualResponse = testPostControllerForAdmin("/api/withDb/1.0/medicines",medicineDto);
    ...
    void getAll() throws Exception {
        actualResponse = testGetControllerForUser("/api/withDb/1.0/medicines");
        ArrayList<MedicineDto> medicineDtoArrayList = new ArrayList<>();
        medicineDto.getIngredients().add(ingredientDto);
        medicineDtoArrayList.add(medicineDto);
        expectedDataResult = new SuccessDataResult(medicineDtoArrayList,"Ä°laÃ§ listesi getirildi.");
        assertThat(actualResponse).isEqualToIgnoringWhitespace(
                objectMapper.writeValueAsString(expectedDataResult));
    ...
    void update() throws Exception {
        medicineDto.setName("Vermidon");
        actualResponse = testPutControllerForAdmin("/api/withDb/1.0/medicines/1",medicineDto);
