...
@RestController
@RequestMapping("${apiDb:}/medicines")
@RequiredArgsConstructor

public class MedicinePgSqlController {
    @Qualifier("pgsqlMedicine") @NonNull private final MedicineService medicineService;
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<DataResult<MedicineDto>> update(@RequestBody MedicineDto medicineDto, @PathVariable Long id) throws MedicineNotFoundException {
        return ResponseEntity.ok(medicineService.update(id,medicineDto));
    }

    @GetMapping("/{id}")
    @PreAuthorize("hasRole('USER')")
    public ResponseEntity<DataResult<MedicineDto>> getById(@PathVariable Long id) throws MedicineNotFoundException {
